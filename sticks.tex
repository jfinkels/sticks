...
\begin{document}
  ...
  \begin{repdefinition}{def:asym-sticks}[Linear programming characterization]
    ...
    \item[] Every tile must start at exactly one position:
      ...
      \end{displaymath}
    \item[] Each position must have at most one color:
      \begin{displaymath}
        \sum_{i = 1}^{|C|}{x_{j, i}} = 1, \plain{for} 1\leq j\leq k
      \end{displaymath}
    \item[] The colors of the layout must correspond to the colors of the
      tiles:
      \begin{displaymath}
        y_{i, j} \leq x_{i + k, s_{j, k}}, \plain{for} 1\leq i\leq k - 2,\;
        1\leq j\leq n,\; 0\leq k\leq 2
      \end{displaymath}
    \end{itemize}
